...
#фильтр ФВЧ на 10 Гц крутизна порядок -  нужно выбрать
#создание загрузочного  файла(текстовый ) с отчетами IQ
...
def main():
    ...
    #interface choose
    if args.socket_addr:
        client = SocketClient(args.socket_addr)# получение сырого потока данных с интерфейса
    elif args.spi:
        client = SPIClient()# получение сырого потока данных с интерфейса
    else:
        port = args.serial_port or example_utils.autodetect_serial_port()
        client = UARTClient(port)# получение сырого потока данных с интерфейса
...
class PhaseTrackingProcessor:
    ...
    def process(self, sweep):
        ...
        if self.sweep_index == 0:
            ...
            plot_data = calculation(n,ampl,power,com)
    ...
    def calculation(self,n,ampl,power,com):
